...
<body>
<div id="wrap">
  <div id="title">KOI 3D Space<div id="sub">features: 10 transit/orbital + 4 FP flags · unsupervised embedding</div></div>
...
<script>
const actualColor = {"0":"#B22222","1":"#228B22"};
...
loadData().then(DATA=>{
  ...
  const matCandidate = new THREE.MeshBasicMaterial({ color: 0xB22222 });
  const matConfirmed = new THREE.MeshBasicMaterial({ color: 0x228B22 });
  ...
  function onMove(e){
    ...
    if (hits.length){
      ...
      tip.innerHTML = `<strong>${d.kepler_name || '—'}</strong><br>
                       kepid: ${d.kepid}<br>
                       label: ${d.actual_label_name}<br>
                       cluster: ${d.cluster}<br>
                       prad: ${Number.isFinite(d.koi_prad)?d.koi_prad.toFixed(2):'—'}, teq: ${Number.isFinite(d.koi_teq)?d.koi_teq.toFixed(0):'—'}`;
...
PORT = 8008
os.chdir(args.out_dir)  # make sure viewer.html and koi_tsne3d_points.json are here
